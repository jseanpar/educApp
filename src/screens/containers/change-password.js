...
class ChangePassword extends Component {
    ...
    state = { loading: true, password: '', passwordConfirm: '' }
   ...
   handleButtonClick = async () => {
        ...
        if ( ( typeof this.state.passwordConfirm === 'undefined') || ( this.state.passwordConfirm === '' ) ) {
            //this.setState ( { loading: false } )
            this.showMessage('Atención', 'Debe confirmar la contraseña.')
            return false
        }

        if ( this.state.password !== this.state.passwordConfirm ) {
            //this.setState ( { loading: false } )
            this.showMessage('Atención', 'las contraseñas no coinciden.')
            return false
        }
        else { 
            await API.getAuth()
            .then( ( auth ) => {  
                API.updateUserPassword ( auth, this.props.user.userCardId, MD5(this.state.password), 'N' )
                .then( ( result ) => {
                    this.props.dispatch({ type : 'REMOVE_USER', })
    ...
    render () {
        ...
        return (
            <Container>
                ...
                <Header title = 'Cambio de contraseña' navigation = { this.props.navigation } showMenu = { false } >
                    <HeaderBackButton onPress = { this.handleHeaderBackButton } />
                </Header>
                <View style={{ flex: 1, margin: 10 }}>
                    <Form >
                        ...
                        <Item floatingLabel last>
                            ...
                            <Input secureTextEntry = { true } value = { this.state.passwordConfirm } onChangeText = { (text) => this.setState({ passwordConfirm: text }) } />
                            <Icon active name='md-key'/>
                        </Item>
